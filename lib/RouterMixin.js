...
var RouterMixin = {
  ...
  propTypes: {
    ...
    onProposeNavigation: React.PropTypes.func,
  ...
  proposeNavigate: function(path, navigation, cb) {
    var state = this.getRouterState(this.props);
    state.navigation = navigation;

    if (this.props.onProposeNavigation &&
        this.props.onProposeNavigation(state.path, navigation, state.match, path) === false) {
      return false;
    }

    if (navigation.onProposeNavigation &&
        navigation.onProposeNavigation(state.path, navigation, state.match, path) === false) {
      return false;
    }
