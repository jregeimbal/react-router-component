...
Environment.prototype.proposeNavigate = function proposeNavigate(path, navigation, cb) {
  var latch = this.routers.length;
  ...
  var canINavigate = true;

  batchedUpdates(function() {
    for (var i = 0, len = this.routers.length; i < len; i++) {
      if (this.routers[i].proposeNavigate(path, navigation, callback) === false) {
        canINavigate = false;
      };
    }
  }.bind(this));
...
Environment.prototype.setPath = function(path, navigation, cb) {
  ...
  var canINavigate = this.proposeNavigate(path, navigation, cb);
