...
Environment.prototype.proposeNavigate = function proposeNavigate(navigation, cb) {
  var latch = this.routers.length;
  ...
  batchedUpdates(function() {
    for (var i = 0, len = this.routers.length; i < len; i++) {
      if (this.routers[i].proposeNavigate(this.path, navigation, callback) === false) {
        canINavigate = false;
      };
    }
  }.bind(this));
...
Environment.prototype.setPath = function(path, navigation, cb) {
  ...
  var canINavigate = this.proposeNavigate(navigation, cb);
